...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Custom CSS set in param args.csspath -->
        ...
        <head>
          ...
          <title>
            ...
            <xsl:if test="string-length($ARGS_CSS) > 0">
                <link rel="stylesheet" type="text/css" href="{$ARGS_CSS}">
                    <xsl:text xml:space="preserve"> </xsl:text>
                </link>
            </xsl:if>
      ...
      <div class="legal" xmlns="http://www.w3.org/1999/xhtml">
        <div class="legalCopyright">
          ...
          <xsl:if test="string-length($toc/toc:copyright) > 0">
            <br/>
            <xsl:value-of select="$toc/toc:copyright"/>
          </xsl:if>
        </div>
        <xsl:if test="string-length($toc/toc:legalnotice) > 0">
          <div class="legalNotice">
            <xsl:value-of select="$toc/toc:legalnotice"/>
          </div>
        </xsl:if>
      </div>
    </xsl:if>    
  ...
  <xsl:template name="create-js-scripts">
    <xsl:param name="withFrames"/>
      <script xmlns="http://www.w3.org/1999/xhtml" type="text/javascript" src="oxygen-webhelp/resources/js/jquery.js"><xsl:comment/></script>
    ...
    <xsl:if test="not($withFrames)">
        <script xmlns="http://www.w3.org/1999/xhtml" type="text/javascript" charset="utf-8" src="oxygen-webhelp/resources/js/jquery.ba-hashchange.js"><xsl:comment/></script>
